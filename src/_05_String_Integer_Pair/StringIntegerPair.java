...
public class StringIntegerPair {
	...
	public void put(String key, int value) {
		...
		String[] oneElementLarger = new String[keys.length + 1];

		// C. create an integer array that is one element longer than values

		int[] oneElementLargerInteger = new int[values.length + 1];

		// D. set the last element of the new String array to the passed in key

		oneElementLarger[oneElementLarger.length - 1] = key;

		// E. set the last element of the new int array to the passed in value

		oneElementLargerInteger[oneElementLargerInteger.length - 1] = value;
		...
		for (int i = 0; i < keys.length; i++) {
			...
			oneElementLarger[i] = keys[i];
		}

		for (int i = 0; i < values.length; i++) {

			oneElementLargerInteger[i] = values[i];
		}

		// G. Set the keys and values arrays equal to the new arrays created in steps B
		// and C.

		keys = oneElementLarger;
		values = oneElementLargerInteger;
	}

	// // 5. Complete the method so it returns the value located at the passed in
